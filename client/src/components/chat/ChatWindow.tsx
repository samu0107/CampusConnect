...
export default function ChatWindow({ accommodationId, onClose }: Props) {
    ...
    const init = async () => {
      try {
        ...
        const { data: msgData } = await axios.get(
          `${API}/api/chat/${convData.data._id}/messages`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        setMessages(msgData.data);

        // Mark as read
        await axios.put(`${API}/api/chat/${convData.data._id}/read`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        socket.emit("join_conversation", convData.data._id);
  ...
  const sendMessage = () => {
    ...
    setInput("");
    socketRef.current?.emit("stop_typing", { conversationId: conversation._id, userId: user._id });
  };

  const handleTyping = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInput(e.target.value);
    socketRef.current?.emit("typing", { conversationId: conversation?._id, userId: user._id });
    setTimeout(() => {
      socketRef.current?.emit("stop_typing", { conversationId: conversation?._id, userId: user._id });
    }, 1500);
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-md h-[520px] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-slate-700 text-sm">{conversation?.ownerId?.name || "Owner"}</p>
